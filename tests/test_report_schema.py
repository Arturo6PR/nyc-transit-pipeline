...
def test_emitted_reports_match_versioned_json_schema(
    feed_file: Path, schedule_dir: Path, tmp_path: Path
) -> None:
    schema_path = Path(__file__).parents[1] / "docs" / "report-schema-v1.1.json"
    ...
    schedule = ingest_schedule(str(schedule_dir), database=database, source_label="mta-test")
